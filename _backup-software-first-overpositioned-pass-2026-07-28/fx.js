...
(function () {
  ...
  var pageStories = {
    ...
    software: {
      ...
      stages: ['Analizza', 'Progetta', 'Integra', 'Evolve']
    },
    'ai-adoption': {
  ...
  function updateProgress() {
    ...
    body.classList.toggle('journey-start', p < .025);
  ...
  if (finePointer) {
    ...
    /* Tactile abstract artworks: a light, dependency-free parallax replaces
       a heavier WebGL/Three.js scene while preserving the same sense of depth. */
    var abstractHeroes = document.querySelectorAll('.hero-photo');
    Array.prototype.forEach.call(abstractHeroes, function (artwork) {
      artwork.addEventListener('pointermove', function (event) {
        var rect = artwork.getBoundingClientRect();
        var nx = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
        var ny = Math.max(0, Math.min(1, (event.clientY - rect.top) / rect.height));
        artwork.style.setProperty('--photo-x', ((.5 - nx) * 10).toFixed(2) + 'px');
        artwork.style.setProperty('--photo-y', ((.5 - ny) * 10).toFixed(2) + 'px');
        artwork.style.setProperty('--glow-x', (nx * 100).toFixed(1) + '%');
        artwork.style.setProperty('--glow-y', (ny * 100).toFixed(1) + '%');
      });
      artwork.addEventListener('pointerleave', function () {
        artwork.style.setProperty('--photo-x', '0px');
        artwork.style.setProperty('--photo-y', '0px');
        artwork.style.setProperty('--glow-x', '50%');
        artwork.style.setProperty('--glow-y', '50%');
      });
    });
